...
function Delete($array)
	...
	if (!$r)
		...
		if (!$row)
			...
			echo "<script type='text/javascript'>";
			echo "alert ('This Subscribers does not exist. You can not delete it.')";
			echo "</script>";
			mysql_close($link);
			return;
			}
			...
			$sqlDel = "DELETE FROM Subscribers WHERE SubscriberID = " . $array[0];
			...
			echo "}else{";
			echo "alert ('Subscribers Update Canceled.')";
			echo "};";
			echo "</script>";
			mysql_close($link);
			return $row;
